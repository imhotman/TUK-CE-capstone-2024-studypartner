...
class UploadFile(models.Model):
    ...
    lecture = models.ForeignKey(LectureChapter, related_name='uploaded_files', on_delete=models.CASCADE, null=True)  # 강의 정보를 저장하는 외래 키
    chapter = models.ForeignKey(LectureChapter, on_delete=models.CASCADE)  # 챕터 정보를 저장하는 외래 키
    file_title = models.CharField(default="", max_length=50)  # 파일 제목
    file_name = models.FileField(null=True)  # 실제 파일

    def __str__(self):
        return f"사용자={self.user}, 강의명={self.lecture.lecture.title}, 챕터명={self.chapter.chapter_name}, 파일 제목={self.file_title}, 파일 이름={self.file_name}"
